...
class FR3MuJocoEnv:
    ...
    def reset(self, q_nominal=[0.0, -0.79, 0.0, -2.36, 0.0, 1.57, 0.79, 0.01, 0.01]):
        for i in range(9):
    ...
    def visualize_object(self, q, base_T_obj):
        for i in range(7):
            self.data.qpos[i] = q[i]
            self.data.qvel[i] = 0.0
        self.data.qpos[7] = 0.01
        self.data.qvel[7] = 0.0
        self.data.qpos[8] = 0.01
        self.data.qvel[8] = 0.0
        _TW = np.hstack((self.base_R_offset, self.base_p_offset))
        world_T_base = np.vstack((_TW, np.array([[0.0, 0.0, 0.0, 1.0]])))
        base_T_obj = world_T_base @ base_T_obj
        obj_p = base_T_obj[:3, 3]
        # obj_R = base_T_obj[:3, :3]
        # obj_quat = Rotation.from_matrix(obj_R).as_quat()
        self.data.qpos[9:12] = obj_p
